...
// -------------------- Advanced optimizer helpers (experimental) --------------------
const getChosenSG = (selected, typePicks) => {
  return ORDER
    .filter(k => selected[k])
    .flatMap(level => {
      ...
      return flt.map(u => {
        const unitStrength = getSGStrength(level, u.type);
        const ratio = u.health > 0 ? unitStrength / u.health : 0;

        // Approximation expérimentale, pas la formule exacte de TotalCalculator.
        // Sert à donner un ordre plus proche du principe "effective strength".
        const effectiveStrength = unitStrength * (1 + Math.min(ratio, 1));
...
const computeAdvancedSG = (leadership, selected, typePicks) => {
  const chosen = getChosenSG(selected, typePicks);
...
// -------------------- Calcs --------------------
    ...
    const computeSG = (leadership, selected, typePicks) => {
      ...
      return { rows, used, leftover: leadership - used, T, SR, advancedOrder: chosen };
    };
...
export default function TBStackCalculator(){
  ...
  // Mode de calcul
  const [calcMode, setCalcMode] = useState("balanced");
...
<section className={`${clsPanel} mx-2`}>
  <h2 className="text-lg mb-3" style={glow}>CALCULATION MODE</h2>
  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
    <button
      type="button"
      className={btnCls(calcMode === "balanced")}
      onClick={() => { setCalcMode("balanced"); sfx.select(); }}
    >
      Balanced HP
    </button>
    <button
      type="button"
      className={btnCls(calcMode === "advanced")}
      onClick={() => { setCalcMode("advanced"); sfx.select(); }}
    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <ul className="list-disc pl-6">
                  <li>Mode = {calcMode}</li><li>L = {fmtInt(L)}</li><li>SR = {sg.SR.toFixed(6)} (Σ ldr/HP)</li><li>T = {fmtInt(sg.T)}</li><li>Used L = {fmtInt(sg.used)}</li><li>Leftover L = {fmtInt(sg.leftover)}</li>
                </ul>
              ...
              <div>
                ...
                <h3 className="font-semibold mt-3 mb-1" style={glow}>Advanced order</h3>
                <ul className="list-disc pl-6">
                  {sg.advancedOrder?.length ? sg.advancedOrder.slice(0,30).map((r,i)=>(
                    <li key={i}>{i + 1}. {r.level} · {r.type}: ratio={(r.ratio ?? 0).toFixed(6)} · eff={(r.effectiveStrength ?? 0).toFixed(2)}</li>
                  )) : <li>—</li>}
                </ul>
